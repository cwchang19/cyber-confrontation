    ...
    <el-form
      ...
      <el-form-item 
        ...
        <template>
          ...
          -->
          <el-input
            v-if="item.key !== 'algorithm_file'"
            v-model="algorithmForm[item.key]"
            :type="item.key === 'algorithm_name' ? '' : 'textarea'"
      ...
      <el-form-item>
        <el-button type="primary" @click="formBtnClick('confirm')">立即创建</el-button>
  ...
  export default {
    ...
    methods: {
      fetchData() {
        ...
        // 下面这里只是假装请求到数据了
        this.algorithmForm.id = id;
        this.algorithmForm.algorithm_name = id;
        this.algorithmForm.algorithm_description = id;
        this.algorithm_file = id;
      },
      // 表单的确认和取消按钮的点击事件
      formBtnClick(type) {
