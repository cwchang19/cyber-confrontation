<template>
  <div class="action-container">
          ...
          <el-table :data="tableData" style="width: 100%">
            <el-table-column v-for="(item, index) in tableColumns" :prop="item.prop" :label="item.label"
              ...
              <template v-if="item.prop === 'operate'" v-slot="scope">
                ...
                <el-button type="danger" size="mini" @click="deleteActDialogVisible = true; selectedActId = scope.row.id">删除
                </el-button>
              </template>
            </el-table-column>
          </el-table>
    ...
    <el-dialog :title="isEdit ? '编辑动作' : '新增动作'" v-if="formDialogVisible" :visible.sync="formDialogVisible"
      width="37.5rem">
...
<script>
import { searchActionByCondition, addAction, alterAction, deleteAction, searchActionById } from '@/api/action'
...
export default {
  ...
  methods: {
    ...
    async editClick(row) {
      ...
      this.formDialogVisible = true
      const response = await searchActionById(this.selectedActId)
      this.actionForm = response.data
    },
    async deleteClick() {
      // 根据id请求后端删除对应算法
      const response = await deleteAction(this.selectedActId);
    ...
    dialogClick() {
      this.$refs['actionForm'].validate(async (valid) => {
        if (valid) {
          if (this.isEdit) {
            const response = await alterAction(this.selectedActId, this.actionForm)
          } else {
