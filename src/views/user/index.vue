<template>
  <div class="user-container">
          ...
          <el-table :data="tableData" style="width: 100%">
            <el-table-column v-for="(item, index) in tableColumns" :prop="item.prop" :label="item.label"
              ...
              <template v-if="item.prop === 'operate'" v-slot="scope">
                ...
                <el-button type="danger" size="mini" @click="deleteUserDialogVisible=true; selectedUserId=scope.row.id">删除</el-button>
              </template>
            </el-table-column>
          </el-table>
        </el-row>
    ...
    <el-dialog :title="isEdit ? '编辑用户' : '新增用户'" v-if="formDialogVisible" :visible.sync="formDialogVisible" width="37.5rem">
...
<script>
import { searchUserByCondition, addUser, alterUser, deleteUser } from '@/api/userM';
export default {
  name: 'User',
  components: {},
  data() {
  ...
  methods: {
    ...
    async editClick(row) {
      ...
      this.formDialogVisible = true;
      const response = await searchUserByCondition({ id: this.selectedUserId });
      if(response.data.total > 0) {
        this.userForm = response.data.items[0];
      }
    },
    async deleteClick() {
      const response = await deleteUser({ id: this.selectedUserId });
    ...
    dialogClick() {
      this.$refs['userForm'].validate(async (valid) => {
        if(valid) {
          if(this.isEdit) {
            const response = await alterUser({ id: this.selectedUserId } ,this.userForm);
          } else {
